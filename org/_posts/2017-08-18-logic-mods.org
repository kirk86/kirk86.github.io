...
- ~Composition~: $S\circ R$, also denoted $R; S (or R\circ S)$,
  defined as $S\circ R =
{ (x, z) | \exists y\in Y$, such that $(x, y)\in R\wedge (y, z)\in
S }$. The order of $R$ and $S$ in the notation $S\circ R$, used here agrees
